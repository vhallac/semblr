...
2. **Embed prompt, response, and combined text.** Three texts are sent to the configured embedding API: the user prompt (noise-collapsed first: large code fences, JSON dumps, and zero-variety repetition runs are replaced by short placeholders; the raw prompt stays verbatim in the round file), the clipped response (by default truncated to ~24KB, context-injection artifacts stripped), and the concatenation of both (`cleanedPrompt + "\n\n" + clippedResponse`). The prompt and response vectors are stored in an append-only CSV index. The combined vector is stored in the round file for semantic grouping.
