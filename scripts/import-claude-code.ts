...
function roundFileName(round: Pick<Round, "userPrompt" | "responseSequence">): string {
  return `${crypto.createHash("md5").update(round.userPrompt + round.responseSequence).digest("hex")}.json`;
}
...
function parseClaudeFile(filePath: string): Round[] {
  ...
  const entries = raw.split("\n").filter(Boolean).map((line, i) => {
    ...
    catch (e) { throw new Error(`${filePath}:${i + 1}: invalid JSON: ${(e as Error).message}`); }
  });
  ...
  function flush() {
    ...
    const round: Round = {
      id: crypto.createHash("md5").update(userPrompt + responseSequence).digest("hex"),
      source: "claude-code",
      userPrompt,
      responseSequence,
      responseSegments,
...
function loadIndexedRoundFiles(): Set<string> {
  ...
  return new Set(fs.readFileSync(INDEX_PATH, "utf-8")
    .split("\n")
    .filter(Boolean)
...
async function main() {
  ...
  for (const file of files) {
    try { allRounds.push(...parseClaudeFile(file)); }
    catch (e) { parseErrors++; console.warn(`⚠ ${(e as Error).message}`); }
  }
  ...
  let newRounds = allRounds.filter((round) => {
    ...
    if (seen.has(file)) { duplicates++; return false; }
    seen.add(file);
    if (indexed.has(file)) { alreadyIndexed++; return false; }
    return true;
  });
  if (LIMIT != null && Number.isFinite(LIMIT)) newRounds = newRounds.slice(0, LIMIT);

  console.log(`📊 Parsed rounds: ${allRounds.length}`);
  console.log(`📊 New rounds to import: ${newRounds.length} (${alreadyIndexed} already indexed, ${duplicates} duplicate in source, ${parseErrors} parse errors)`);
  ...
  const vectors = fs.existsSync(INDEX_PATH) ? fs.readFileSync(INDEX_PATH, "utf-8").split("\n").filter(Boolean).length : 0;
